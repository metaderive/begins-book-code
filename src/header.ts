 ...
 *   → バイト境界まで0詰め（page1 が空なら page1 セクションは書かない＝可変長2〜6Bの正体）
 *
 * ×3 と ×4 は独立フラグを持たず「高位フラグ H」を共有し、H が立つと n4(4bit)+n3(4bit) の
 * 8bit ブロックを必ずペアで書く（片方0でも両方書く）。実機13本で確定:
 ...
 *     実機 `60 01 0d`（NEWp0 {1:[1,0],3:[13,0]}＝全カードが page0）の先頭 `60` が 0 の唯一の例で、
 *     他の12本（全て page1 あり）は 1。
 ...
 *     した（2026-09-12）。
 *     未検証: 「ページ1が空で page0 に ×4 がある／×3 が無い」構成の実機サンプルは無い
 *     （本規則の予測は先頭 0。旧規則なら 1 で、両規則が食い違う構成）。
